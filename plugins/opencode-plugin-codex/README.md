...
- `cwd` is realpath-contained inside roots supplied by standard MCP roots, current Codex per-call workspace metadata, the matching persisted current-thread rollout cwd, or explicit `OPENCODE_WORKSPACE_ROOTS` for ephemeral tasks. The configured value uses the platform path delimiter. An explicit `cwd` never becomes a trusted root by itself. `files` remains inside `cwd`. Configure the CLI with trusted `OPENCODE_BIN`, not a tool parameter.
