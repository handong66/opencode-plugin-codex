...
`callerCwd` reports only presence, syntactic acceptance, accepted count, and
`invalid_caller_cwd`. Never add thread IDs, environment values, URIs, paths, raw metadata, rollout contents,
...
| --- | --- | --- |
| `workspace_unavailable` | no | no explicit absolute `cwd` and no usable MCP, Codex-session, or configured workspace root |
| `workspace_out_of_bounds` | no | an internally resolved default or protected path resolves outside every available root |
...
The probe runs on the **submit** path (`opencode_run`, `opencode_continue`, `opencode_rescue`, the two reviews) only when the caller passed an explicit `model` — with nothing requested there is nothing to compare, and an unconditional probe would add a CLI process to the path 943 of 1,051 recorded jobs took. Two paths probe unconditionally, for reasons that are not comparison: `opencode_check`, which reports `effectiveModel` as a diagnostic, and `opencode_transfer`, whose imported session file has to name a model whether or not the caller asked for one. `test/model-guard.test.ts` pins all three cases against a CLI that records its own argv, so this paragraph cannot drift from the code again.
...
A record is monotonic: `JobStore.write()` ignores a write that would put a non-terminal status over a `succeeded`, `failed` or `cancelled` record — the same rule the cancellation sentinel already applied to one status. Every writer in this codebase does a read-modify-write, and the worker's throttled progress write is fire-and-forget, so a write that began before the job ended could otherwise land after the terminal write and put `running` back on top of a result. The worker exits, and the next `status()` poll finds a `running` record with a dead `workerPid` and rewrites it as `worker_unavailable` — a completed result reported as a dead worker. The worker also serialises its progress writes on one chain and awaits that chain before finalising, so the two halves of the guard are independent.

The same staleness reaches one field further down, and `JobStore.write()` covers that too: a stored `pid` or `workerPid` survives a write that carries none. The worker records the OpenCode child's pid immediately after spawning it, and a progress write whose read happened just before that could otherwise replay a record with no pid at all — which is the handle `JobStore.cancel` signals first, and the only route to a detached child once the worker itself is gone. Nothing in this codebase ever wants to unset a pid, so the rule costs no legitimate write; a deliberate re-point (the OX1 answer pass) sets a new value rather than clearing the old one.

There is one incremental reader on the hot path, and only one: `readStreamProgress()` (same module), which `job-worker.ts` calls per stdout chunk to feed the watchdog's tool-call count and to enforce `maxToolCalls` when the caller set one. A half-written JSONL line still cannot be parsed, so the worker buffers the trailing partial line and hands `readStreamProgress()` complete lines only; it extracts nothing but the tool-call delta and the session id. Everything else about a record's shape stays in the single end-of-stream pass — do not grow this reader into a second finalizer.

Reaching `maxToolCalls` interrupts the first pass and runs a second one against the same session, whose budget is `finalAnswerBudgetMs()` in `src/job-finalize.ts`: the unspent remainder of `timeoutMs`, capped at `FINAL_ANSWER_MAX_MS` (120000) and floored at `FINAL_ANSWER_MIN_MS` (30000). The floor is the only path by which a job outlives its `timeoutMs`, and it can do so by at most 30s plus the 2s SIGKILL grace. Keep it that way: the floor exists because giving the answer pass the 3s that happened to be left would discard the work the interrupt was meant to save, and the bound is published on the `maxToolCalls` parameter and pinned by `test/tool-call-budget.test.ts`, so changing either constant means changing both.
