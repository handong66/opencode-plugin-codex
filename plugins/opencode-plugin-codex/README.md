...
- `cwd` is realpath-contained inside roots supplied by standard MCP roots or current Codex per-call workspace metadata. Decoded-object and bounded JSON-string metadata are accepted; an explicit `cwd` never becomes a trusted root by itself. `files` remains inside `cwd`. Configure the CLI with trusted `OPENCODE_BIN`, not a tool parameter.
