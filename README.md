...
- An explicit absolute `cwd` is accepted as the workspace root and must resolve to an existing directory. This restores compatibility with Codex tasks that provide neither standard MCP roots nor workspace metadata, including bare ephemeral tasks. When `cwd` is omitted, the plugin falls back to standard MCP roots, current Codex per-call workspace metadata, the matching persisted current-thread rollout cwd, and explicit `OPENCODE_WORKSPACE_ROOTS`. The environment value uses the platform path delimiter (`:` on macOS/Linux, `;` on Windows). This is an intentional capability tradeoff: the MCP caller can run OpenCode in any directory accessible to the plugin's OS user. `files` still accepts at most 32 existing regular files whose real paths stay inside `cwd`; outside paths and escaping symlinks are rejected.
...
- `opencode_check` is cached for the MCP server process (`force: true` re-reads) and reports `workspace`, path-free `workspaceSources`, `effectiveModel`, `providers`/`providerIds`/`models` as parsed ANSI-free arrays, `agents`, the inherited `proxy` variables (any userinfo masked as `***`), and any leftover 0.1-era `.opencode-plugin-codex` / `.grok-plugin-codex` directory in the workspace (reported, never deleted). `workspaceSources` distinguishes standard roots, request metadata, persisted session cwd, configured roots, and explicit caller cwd using counts and normalized status only—never paths, thread IDs, raw metadata, or environment values.
...
Boundary refusals are returned, not thrown. A relative, missing, or non-directory `cwd` is rejected; attachment paths must stay inside its resolved directory. `workspace_out_of_bounds` remains relevant to internally resolved defaults and protected transfer paths, while an explicit absolute `cwd` authorizes its own directory. The codes are `workspace_unavailable`, `workspace_out_of_bounds`, `file_attachment_invalid`, `private_path_blocked`, `rollout_invalid`, `state_write_failed`, `cli_not_found`, `cli_probe_timeout`, `job_not_found`, plus the `errorClass` vocabulary for OpenCode's own failures and the per-tool codes listed in `plugins/opencode-plugin-codex/skills/opencode/references/failure-routing.md`. On `opencode_status`, `opencode_result`, and `opencode_cancel`, `ok` describes the **job's** outcome, not the query's: a `failed` or `cancelled` job returns `ok: false` with `error: { code, message, retryable }`, and `terminal`/`nextAction` say whether the record can still change.
