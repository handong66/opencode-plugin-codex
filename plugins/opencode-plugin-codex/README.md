...
- An explicit absolute `cwd` is accepted as a per-call workspace grant and must resolve to an existing directory, including when Codex supplies no roots or advertises an unrelated root. The grant is not remembered. When omitted, it falls back to standard MCP roots, current Codex per-call workspace metadata, the matching persisted current-thread rollout cwd, or `OPENCODE_WORKSPACE_ROOTS`. This intentionally lets the MCP caller run OpenCode in any directory accessible to the plugin's OS user. `files` remains realpath-contained inside `cwd`. Configure the CLI with trusted `OPENCODE_BIN`, not a tool parameter.
...
- Every tool returns `{ ok, error?: { code, message, retryable, details? }, warnings: string[], data }`. The payload is in `data`; small scalars (`terminal`, `nextAction`, `waited`, `resumable`, `errorClass`, `modelSelection`, …) are mirrored at the top level for the 0.2 transition, bulk fields are not. `opencode_check.data.workspaceSources` diagnoses trusted roots without returning paths or raw metadata. Boundary refusals are returned with a stable code, not thrown. Read payloads from `structuredContent`; above 8192 characters they are not duplicated as text.
