...
- `opencode_check` is cached for the MCP server process (`force: true` re-reads) and reports `workspace`, `effectiveModel`, `providers`/`providerIds`/`models` as parsed ANSI-free arrays, `agents`, the inherited `proxy` variables (any userinfo masked as `***`), and any leftover 0.1-era `.opencode-plugin-codex` / `.grok-plugin-codex` directory in the workspace (reported, never deleted).
