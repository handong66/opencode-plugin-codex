...
`errorClass: "stalled"` is not a spent budget: the worker ended a run that produced almost nothing and then went silent for 45 seconds. That is a provider or model hang — retry with a lighter explicit model or check the provider, credentials, and proxy; a larger `timeoutMs` repeats it. A `timeout` with zero tool calls gets the same guidance for the same reason. `lastEventAt` on the record says when output last arrived, so "still working" and "silent since" are distinguishable while the job runs.
