...
- `cwd` must resolve inside a trusted filesystem root. Sources, in order, are standard MCP roots, current Codex per-call workspace metadata, the matching persisted current-thread rollout cwd, and explicit `OPENCODE_WORKSPACE_ROOTS` for ephemeral tasks. The environment value uses the platform path delimiter (`:` on macOS/Linux, `;` on Windows). An explicit tool `cwd` never authorizes itself. `files` accepts at most 32 existing regular files whose real paths stay inside `cwd`; outside paths and escaping symlinks are rejected.
