...
- Use `opencode_review` or `opencode_adversarial_review` for read-only second-agent analysis; they do not accept `autoApprovePermissions` at all. Use `opencode_rescue` for a second opinion on why something fails — its prompt tells OpenCode to stay read-only, but that is an instruction, not a sandbox, and it does accept `autoApprovePermissions`, which also approves writes. Leave that unset for a diagnosis.
...
`opencode_check` also reports path-free `workspaceSources`, `agents`, the `proxy` variables OpenCode will inherit (credentials in a proxy URL are masked), and any leftover 0.1-era `.opencode-plugin-codex` / `.grok-plugin-codex` directory in the workspace (safe to delete, never deleted for you). `workspaceSources` distinguishes standard roots, request metadata, a matching persisted-session cwd, `OPENCODE_WORKSPACE_ROOTS`, and explicit caller cwd using counts and normalized status only. It never contains thread IDs, workspace paths, environment values, raw metadata, rollout contents, or client error text. In an ephemeral task with no persisted session, pass the project as an explicit absolute `cwd`; configured roots remain available when a call omits `cwd`. A configured proxy is the first thing to test when provider calls fail or hang.
...
`opencode_run` defaults to background execution. Put task text in `prompt`; it is sent through stdin. `files` accepts at most 32 existing regular files whose real paths remain inside `cwd`. An explicit absolute `cwd` is accepted as a per-call workspace grant, must resolve to an existing directory, works with empty or unrelated advertised roots, and is not remembered. This is an intentional capability tradeoff: the MCP caller can run OpenCode anywhere the plugin's OS user can access. When `cwd` is omitted, standard MCP roots, current Codex per-call workspace metadata, the matching persisted-session cwd, or `OPENCODE_WORKSPACE_ROOTS` supply the default. Configure a nonstandard CLI through the trusted MCP environment variable `OPENCODE_BIN`; tools do not accept a caller-controlled executable path.
