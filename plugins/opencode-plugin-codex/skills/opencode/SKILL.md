...
`opencode_run` defaults to background execution. Put task text in `prompt`; it is sent through stdin. `files` accepts at most 32 existing regular files whose real paths remain inside `cwd`. `cwd` must remain inside a trusted root supplied by standard MCP roots, current Codex per-call workspace metadata, the matching persisted-session cwd, or `OPENCODE_WORKSPACE_ROOTS`; a caller-controlled `cwd` is never a root by itself. Configure a nonstandard CLI through the trusted MCP environment variable `OPENCODE_BIN`; tools do not accept a caller-controlled executable path.
